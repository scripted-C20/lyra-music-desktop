...
function processFile(filepath) {
    ...
    if (!content.includes('<app-icon')) return;

    const matches = [...content.matchAll(/<app-icon\s+[^>]*name=([^ >/]+)[^>]*\/?>(<\/.+?>)?/g)];
    if (matches.length > 0) {
        let replacedContent = content;
        let addedIcons = new Set();

        // Simple replacement
        replacedContent = replacedContent.replace(/<app-icon(.*?)name=['\"]([^'\"]+)['\"](.*?)(\/?)>/g, (match, before, name, after, selfClose) => {
