...
import { buildLyricConfig, getAbsoluteLyricWindowBounds } from './utils'
import { sendNewDesktopLyricClient } from '@main/modules/winMain'
import { getBounds, getMainFrame, sendEvent, setBounds, setRendererWindowInteractionState, setResizeable, setWindowPosition, startWindowInteraction, stopWindowInteraction } from './main'
...
export default () => {
  ...
  mainOn<LX.DesktopLyric.NewBounds>(WIN_LYRIC_RENDERER_EVENT_NAME.set_win_bounds, ({ params: options }) => {
    ...
    const nextBounds = getAbsoluteLyricWindowBounds(currentBounds, options)
    if (options.w <= 0 && options.h <= 0) {
      setWindowPosition(nextBounds.x, nextBounds.y)
      return
    }
    setBounds(nextBounds)
  })

  mainOn<LX.DesktopLyric.WindowInteractionOrigin | null>(WIN_LYRIC_RENDERER_EVENT_NAME.set_win_interaction_state, ({ params: origin }) => {
    setRendererWindowInteractionState(origin)
  })

  mainOn<LX.DesktopLyric.StartWindowInteraction>(WIN_LYRIC_RENDERER_EVENT_NAME.start_win_interaction, ({ params }) => {
    startWindowInteraction(params.origin, params.screenX, params.screenY)
  })

  mainOn(WIN_LYRIC_RENDERER_EVENT_NAME.stop_win_interaction, () => {
