...
export default () => {
  ...
  mainHandle<boolean>(WIN_MAIN_RENDERER_EVENT_NAME.max, async() => {
    return maximize()
  })
  ...
  mainOn<boolean>(WIN_MAIN_RENDERER_EVENT_NAME.set_window_resizable, ({ params: isResizable }) => {
    setWindowResizable(isResizable)
  })

  mainOn<{ screenX: number, screenY: number }>(WIN_MAIN_RENDERER_EVENT_NAME.start_window_move, ({ params }) => {
    startWindowMove(params.screenX, params.screenY)
  })

  mainOn(WIN_MAIN_RENDERER_EVENT_NAME.stop_window_move, () => {
    stopWindowMove()
  })
