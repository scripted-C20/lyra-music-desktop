...
export const setWindowBounds = (params: Partial<Electron.Rectangle>) => {
  rendererSend(WIN_MAIN_RENDERER_EVENT_NAME.set_window_size, params)
}
export const setWindowResizable = (isResizable: boolean) => {
  rendererSend(WIN_MAIN_RENDERER_EVENT_NAME.set_window_resizable, isResizable)
}
export const startWindowMove = (params: { screenX: number, screenY: number }) => {
  rendererSend(WIN_MAIN_RENDERER_EVENT_NAME.start_window_move, params)
}
export const stopWindowMove = () => {
  rendererSend(WIN_MAIN_RENDERER_EVENT_NAME.stop_window_move)
}
...
export const maxWindow = () => {
  return rendererInvoke<boolean>(WIN_MAIN_RENDERER_EVENT_NAME.max)
}
...
export const onWindowStateChange = (listener: LX.IpcRendererEventListenerParams<{
  isFullscreen: boolean
  isMaximized: boolean
}>): RemoveListener => {
  rendererOn(WIN_MAIN_RENDERER_EVENT_NAME.window_state, listener)
  return () => {
    rendererOff(WIN_MAIN_RENDERER_EVENT_NAME.window_state, listener)
  }
}
