...
const handleScrollY = (element: ScrollElement<HTMLElement>, to: number, duration = 300, fn = noop): Noop => {
  ...
  const clean = () => {
    ...
    if (element.lx_scrollTimeout) window.cancelAnimationFrame(element.lx_scrollTimeout)
  ...
  const animateScroll = (now: number) => {
    element.lx_scrollTimeout = undefined
    const currentTime = Math.min(now - startTime, duration)
    ...
    if (currentTime < duration) {
      element.lx_scrollTimeout = window.requestAnimationFrame(animateScroll)
    } else {
