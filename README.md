...
- 本仓库基于源项目做了 UI、交互与客户端播放链路增强，不代表官方版本。
- 重新整理了主界面、左侧菜单、顶部窗口按钮、底部播放栏、播放详情页、设置页与弹窗样式，并统一了顶部右上角按钮的默认灰底风格。
- 底部播放栏与播放详情页补充了统一的封面加载动画、播放按钮加载态与坏源找歌时的状态提示，减少“正在换源但界面没反应”的割裂感。
- “我的列表”、排行榜、歌单内子歌曲列表补充了搜索、定位当前播放、回到顶部与一键播放等工具区交互，并细化了来源标签、列表信息与按钮同排布局。
- 自定义源弹窗已重做，支持本地 / 订阅分类、本地导入、在线订阅、订阅名称、重新订阅、批量选择、导出压缩包、源编码说明与 FAQ 入口；弹窗禁止点空白关闭。
- 基础设置里新增了“自定义源搜索上限秒数”。
- 播放链路做了客户端增强：本地文件 / 已下载文件优先、当前成功源运行时记忆、失败源排除重试、切源续播、请求代次与资源 URL 防竞态，以及后台窗口场景下的误判保护。
- 新增“播放结束前 10 秒预加载下一首”能力，短于 10 秒的音频不参与；预加载失败会回到正式播放链路 fresh 重取当前歌，而不是直接误跳下一首。
- 歌词加载改为优先跟随当前实际成功播放源，减少切源后歌词空白、歌词错源或和当前实际音频不一致的问题。
- 自定义源测试支持快速测试、严格实播全检、整表批量测试、5 窗隐藏测试池并发，以及停止测试 / 删除源 / 关闭弹窗时自动清理任务。

> 在线音乐播放需要用户自行导入或订阅可用的自定义 API / JS 源；本仓库不内置任何在线音频 API。
